...
func TestClusterCollectorUtilities(t *testing.T) {
	...
	t.Run("ParseNodeState", func(t *testing.T) {
		testCases := []struct {
			input    string
			expected string
		}{
			...
			{"idle+cloud", "idle"},
			{"allocated", "allocated"},
			{"allocated+completing", "allocated"},
			{"mixed", "allocated"},
			{"down", "down"},
			{"drain", "drain"},
		...
		for _, tc := range testCases {
			result := collector.parseNodeState(tc.input)
			if result != tc.expected {
				t.Errorf("parseNodeState(%s) = %s, expected %s", tc.input, result, tc.expected)
			}
		}
	})
